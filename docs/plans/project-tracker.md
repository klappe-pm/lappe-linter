...
2026-07-10 Hardening and UX round. Security audit: 5 findings, 4 confirmed and fixed (KEY_LINE ReDoS, glob ReDoS, git --changed quoted-filename bypass, config writeback race) with regression tests; docs/security.md records the threat model and dispositions. Perf: core bundled from source with tree shaking, CLI bundle minified 411 to 193 KB, suggest caching, targeted key-sort re-render. Docs: README rewritten for lappe-linter, docs/linter-yaml.md added, AGENTS and CLAUDE plan docs corrected. Features: test-file generator into _archive date folder with fixtures derived from rule examples, excluded-folders checklist writing ignore.folders, wand-sparkles ribbon quick action plus hotkey-ready commands, and dec-006 declarative code-checks (bounded regex over fences, three builtins, Lappe tab section, docs/code-checks.md). Suite: 105 suites, 1677 tests green on master.
