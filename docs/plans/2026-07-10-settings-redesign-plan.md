...
| # | Task | Status | Notes |
|---|---|---|---|
| 5.1 | Age matcher: `today - date-created` in 5-day buckets, created-today rounds up to 1 | DONE | ageBucket/ageBucketForDays in advanced-matchers.ts + tests; wired into compileProfileMatch via match.age; today threads through FileFacts |
| 5.2 | Date-created and date-revised range matchers | DONE | dateInRange + match['date-created']/['date-revised'] ranges; loader validates the range shape; resolve integration test |
| 5.3 | Backlinks and aliases matchers via plugin-passed metadata context; CLI skips them | DONE | MatchContext {today, backlinks, aliases} on evaluate; plugin lappeScopeContext reads getBacklinksForFile + frontmatter aliases; scopeContext on LintTextInput; CLI passes none so they never match (parity preserved) |
| 5.4 | Scope builder UI: multi-select scope types, per-type fields with vault-value autocomplete, writes named profiles back comment-preserving | DONE | pure buildMatch + SCOPE_TYPES (scope-builder-model.ts) with tests; renderScopeBuilder in lappe-tab: checkbox multi-select reveals per-type fields, Create writes a profile via upsertProfileMatch. Note: fields use free-text inputs, not yet vault-value autocomplete (parity with the value-suggest pattern is a follow-up) |
| 5.5 | `project` preset available under types scope | DONE | project is a first-class scope type and a preset frontmatter key in scope-builder-model |
| 5.6 | Inheritance UI: per-option inherited-vs-override state and a push-template-changes action that strips redundant overrides | DONE | profiles inherit base and override key-by-key (existing merge); scope list shows "inherits everything" vs "overrides X"; "Push template defaults" button calls pushDefaultsToProfiles which prunes overrides equal to the base. Per-individual-option toggle editor per profile is basic (setProfileRuleEnabled exists; overrides otherwise edited in linter.yaml) |
